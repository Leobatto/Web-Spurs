...
export function ForgotPasswordForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(false);
      setStatus(result.error?.message ?? "Si el email existe, te va a llegar un enlace para volver al vestuario.");
  }
  ...
  return (
    <form onSubmit={submit} className="w-full max-w-md rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h1 className="mt-3 text-3xl font-semibold text-zinc-950">Recuperar acceso</h1>
